...
st.title("🚀 Mental Health Tech Dashboard (Interactive)")
st.markdown(f"ข้อมูลปัจจุบัน: **{len(df)}** รายการ")
    
m1, m2, m3 = st.columns(3)
m1.metric("อายุเฉลี่ย", f"{df['age'].mean():.1f} ปี")
m2.metric("สัดส่วนพนักงาน Tech", f"{(df['tech_role']==1.0).mean()*100:.1f}%")
m3.metric("ทำงาน Remote", f"{df['remote_work'].value_counts().idxmax()}")
